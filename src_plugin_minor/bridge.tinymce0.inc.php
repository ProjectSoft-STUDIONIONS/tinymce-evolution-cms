...
class %lowercase%bridge extends modxRTEbridge
{
	public function __construct($tvOptions=array())
	{
		$bridgeConfig = array(
			// Editor-Settings
			'editorLabel'   => '%uppercase%',           // Name displayed in Modx-Dropdowns - No HTML!
			'skinsDirectory'=> '%skinsDirectory%',      // Relative to plugin-dir
			'skinthemeDirectory'=> 'tinymce/themes', // Relative to plugin-dir
			'editorVersion' => '%version%',              // Version of %uppercase%-Library
			...
			'gSettingsDefaultValues' => array(
				...
				'custom_plugins' => 'advlist autolink lists image charmap print preview hr anchor pagebreak searchreplace wordcount visualblocks visualchars code fullscreen spellchecker insertdatetime media nonbreaking save table contextmenu directionality emoticons template paste textcolor codesample colorpicker textpattern imagetools paste modxlink codemirror',
				...
				'custom_buttons2' => 'link unlink anchor image media codesample table | hr removeformat | subscript superscript charmap | nonbreaking | visualchars visualblocks print preview fullscreen code formatselect | codemirror',
		...
		parent::__construct('%lowercase%', $bridgeConfig, $tvOptions, __FILE__);
	...
	public function bridge_selectorPrefix($selector) {
		...
		if (!$inlineMode) {
			...
			$("#action-save").on("click", triggerSave);
			function triggerSave() {
				[+dataObject+]
				var saving = $.post( "' . $this->pluginParams['base_url'] . 'connector.%lowercase%.saveProcessor.php", data );
	...
	public function bridge_selector($selector) {
		...
		if ($selector === 'initBridge' && !defined('INITBRIDGE_%biguppercase%')) {   // called only once right before looping through $this->pluginParams['elements']
			define('INITBRIDGE_%biguppercase%', 1); // don´t call it at every getEditorScript() / TV-init!
			...
			foreach ($sortElArr as $theme => $elArr) {
				$sortElArr[$theme] = '';
				// return selectors comma-separated per theme as single element for %uppercase% & translate options to new selectors
				$key = '';
				foreach ($elArr as $el) {
