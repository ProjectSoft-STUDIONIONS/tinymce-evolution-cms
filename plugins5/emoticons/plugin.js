...
(function () {
    ...
    var load = function(url) {
    	var doc = domUtils.DOM.doc;
    	var head = doc.head;
    	var linkElements = tinyTools.toArray(doc.getElementsByTagName('link'));
    	var matchingLinkElms = tinyTools.grep(linkElements, function (head) {
    ...
    function Plugin () {
      global$3.add('emoticons', function (editor, pluginUrl) {
        ...
        var database = initDatabase(editor, databaseUrl, databaseId);
				// Подключить стили здесь
        register$1(editor, database);
        register(editor);
        init(editor, database);
        setup(editor);
        var cache_suffix = editor.getParam('cache_suffix', '', 'string');
        LoadCss.load(pluginUrl + '/css/main.min.css' + cache_suffix);
      });
    }
