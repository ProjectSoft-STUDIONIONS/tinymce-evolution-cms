...
(function () {
	...
	const myPluginSearchHelper = {
		debounceTimer: null,
		handleSearch: function(dialogApi, query) {
			clearTimeout(this.debounceTimer);
			this.debounceTimer = setTimeout(() => {
				const resultsContainer = document.querySelector('#tinymce-plugin-search-results');
				if (!resultsContainer) {
					console.error(tinymce.translate("Search results container not found for TinyMCE plugin."));
					return;
				}
				if (query && query.trim().length > 0) {
					let num = tinymce.majorVersion;
					fetch(`/assets/plugins/tinymce${num}/tinymce/plugins/modxlink/search.php?q=` + encodeURIComponent(query.trim()))
						.then(response => {
							if (!response.ok) {
						...
						.catch(error => {
							...
							resultsContainer.innerHTML = '<div style="padding:2px 5px; color: red;">' + tinymce.translate('Error loading results:') + ' ' + error.message + '</div>';
						});
				} else {
					resultsContainer.innerHTML = '';
				}
	...
	const makeDialogBody = (urlInput, displayText, titleText, catalogs, hasUploadPanel, fileTypes, onInvalidFiles) => {
		const generalPanelItems = flatten([
			...
			[{
				...
				name: 'search',
				label: 'Search in Evo',
				placeholder: 'Search for resources in Evo'
			}],
			[{
				type: 'htmlpanel',
	...
	const makeDialog = (settings, onSubmit, editor) => {
		...
		return {
			...
			onChange: (api, { name }) => {
				...
				if(name === 'search') {
					myPluginSearchHelper.handleSearch(api, api.getData().search);
				}
			},
			onSubmit
		};
